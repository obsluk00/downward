...
CONFIGS = [
    (f"dfp, no cp", ["--search", "let(h, merge_and_shrink(verbosity=debug, non_orthogonal=false, shrink_strategy=shrink_bisimulation(greedy=false),merge_strategy=merge_stateless(merge_selector=score_based_filtering(scoring_functions=[goal_relevance(), dfp(), single_random()])),label_reduction=exact(before_shrinking=true,before_merging=false),max_states=50k,threshold_before_merge=1), astar(h))"]),
    (f"dfp, online cp", ["--search", "let(h, max_cp_ms(verbosity=debug, non_orthogonal=false, shrink_strategy=shrink_bisimulation(greedy=false),merge_strategy=merge_stateless(merge_selector=score_based_filtering(scoring_functions=[goal_relevance(), dfp(), single_random()])),label_reduction=exact(before_shrinking=true,before_merging=false),max_states=50k,threshold_before_merge=1, cost_partitioning=scp(order_generator=random_orders(random_seed=42)),compute_atomic_snapshot=true,main_loop_target_num_snapshots=0,main_loop_snapshot_each_iteration=0,atomic_label_reduction=false,snapshot_moment=after_label_reduction,filter_trivial_factors=false,offline_cps=false), astar(h))"]),
    (f"dfp, offline cp", ["--search", "let(h, max_cp_ms(verbosity=debug, non_orthogonal=false, shrink_strategy=shrink_bisimulation(greedy=false),merge_strategy=merge_stateless(merge_selector=score_based_filtering(scoring_functions=[goal_relevance(), dfp(), single_random()])),label_reduction=exact(before_shrinking=true,before_merging=false),max_states=50k,threshold_before_merge=1, cost_partitioning=scp(order_generator=random_orders(random_seed=42)),compute_atomic_snapshot=true,main_loop_target_num_snapshots=0,main_loop_snapshot_each_iteration=0,atomic_label_reduction=false,snapshot_moment=after_label_reduction,filter_trivial_factors=false,offline_cps=true), astar(h))"]),
    (f"miasm, no cp", ["--search", "let(h, merge_and_shrink(verbosity=debug, non_orthogonal=false, shrink_strategy=shrink_bisimulation(greedy=false),merge_strategy=merge_stateless(merge_selector=score_based_filtering(scoring_functions=[goal_relevance(), dfp(), sf_miasm(shrink_strategy=shrink_bisimulation(greedy=false), max_states=50000, threshold_before_merge=1), single_random()])),label_reduction=exact(before_shrinking=true,before_merging=false),max_states=50k,threshold_before_merge=1), astar(h))"]),
    (f"miasm, online cp", ["--search", "let(h, max_cp_ms(verbosity=debug, non_orthogonal=false, shrink_strategy=shrink_bisimulation(greedy=false),merge_strategy=merge_stateless(merge_selector=score_based_filtering(scoring_functions=[goal_relevance(), dfp(), sf_miasm(shrink_strategy=shrink_bisimulation(greedy=false), max_states=50000, threshold_before_merge=1),  single_random()])),label_reduction=exact(before_shrinking=true,before_merging=false),max_states=50k,threshold_before_merge=1, cost_partitioning=scp(order_generator=random_orders(random_seed=42)),compute_atomic_snapshot=true,main_loop_target_num_snapshots=0,main_loop_snapshot_each_iteration=0,atomic_label_reduction=false,snapshot_moment=after_label_reduction,filter_trivial_factors=false,offline_cps=false), astar(h))"]),
    (f"miasm, offline cp", ["--search", "let(h, max_cp_ms(verbosity=debug, non_orthogonal=false, shrink_strategy=shrink_bisimulation(greedy=false),merge_strategy=merge_stateless(merge_selector=score_based_filtering(scoring_functions=[goal_relevance(), dfp(), sf_miasm(shrink_strategy=shrink_bisimulation(greedy=false), max_states=50000, threshold_before_merge=1),  single_random()])),label_reduction=exact(before_shrinking=true,before_merging=false),max_states=50k,threshold_before_merge=1, cost_partitioning=scp(order_generator=random_orders(random_seed=42)),compute_atomic_snapshot=true,main_loop_target_num_snapshots=0,main_loop_snapshot_each_iteration=0,atomic_label_reduction=false,snapshot_moment=after_label_reduction,filter_trivial_factors=false,offline_cps=true), astar(h))"])
]
BUILD_OPTIONS = []
