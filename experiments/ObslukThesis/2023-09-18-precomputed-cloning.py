...
CONFIGS = CONFIGS + [(f"depth 0, offline CP", ["--search", "let(h, max_cp_ms(verbosity=normal, non_orthogonal=true, shrink_strategy=shrink_bisimulation(greedy=false), merge_strategy=merge_non_orthogonal_clusters(merge_selector=score_based_filtering(scoring_functions=[goal_relevance(), dfp(), avoid_existing(), single_random()]), tokens=0, combine_strategy=random, cluster_strategy=[pre_eff, eff_eff, eff_pre], depth=0),label_reduction=exact(before_shrinking=true,before_merging=false),max_states=50k,threshold_before_merge=1, cost_partitioning=scp(order_generator=random_orders(random_seed=42)),compute_atomic_snapshot=true,main_loop_target_num_snapshots=0,main_loop_snapshot_each_iteration=0,atomic_label_reduction=false,snapshot_moment=after_label_reduction,filter_trivial_factors=false,offline_cps=true), astar(h))"]),
    (f"depth 0, online CP", ["--search", "let(h, max_cp_ms(verbosity=normal, non_orthogonal=true, shrink_strategy=shrink_bisimulation(greedy=false), merge_strategy=merge_non_orthogonal_clusters(merge_selector=score_based_filtering(scoring_functions=[goal_relevance(), dfp(), avoid_existing(), single_random()]), tokens=0, combine_strategy=random, cluster_strategy=[pre_eff, eff_eff, eff_pre], depth=0),label_reduction=exact(before_shrinking=true,before_merging=false),max_states=50k,threshold_before_merge=1, cost_partitioning=scp(order_generator=random_orders(random_seed=42)),compute_atomic_snapshot=true,main_loop_target_num_snapshots=0,main_loop_snapshot_each_iteration=0,atomic_label_reduction=false,snapshot_moment=after_label_reduction,filter_trivial_factors=false,offline_cps=false), astar(h))"])]
BUILD_OPTIONS = []
