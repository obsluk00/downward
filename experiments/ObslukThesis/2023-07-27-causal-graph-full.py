...
CONFIGS = [
    ...
    (f"clone for each successor", ["--search", "let(h, merge_and_shrink(verbosity=normal, non_orthogonal=true, shrink_strategy=shrink_bisimulation(greedy=false),merge_strategy=merge_stateless(merge_selector=score_based_filtering_explicit_tiebreak(scoring_functions=[goal_relevance(), dfp(), avoid_existing()], tiebreaking_function=single_random())),label_reduction=exact(before_shrinking=true,before_merging=false),max_states=50k,threshold_before_merge=1, tokens=-1, preclone=1), astar(h))"]),
    (f"clone for each predecessor", ["--search", "let(h, merge_and_shrink(verbosity=normal, non_orthogonal=true, shrink_strategy=shrink_bisimulation(greedy=false),merge_strategy=merge_stateless(merge_selector=score_based_filtering_explicit_tiebreak(scoring_functions=[goal_relevance(), dfp(), avoid_existing()], tiebreaking_function=single_random())),label_reduction=exact(before_shrinking=true,before_merging=false),max_states=50k,threshold_before_merge=1, tokens=-1, preclone=2), astar(h))"]),
    (f"clone for both pre- and successor", ["--search", "let(h, merge_and_shrink(verbosity=normal, non_orthogonal=true, shrink_strategy=shrink_bisimulation(greedy=false),merge_strategy=merge_stateless(merge_selector=score_based_filtering_explicit_tiebreak(scoring_functions=[goal_relevance(), dfp(), avoid_existing()], tiebreaking_function=single_random())),label_reduction=exact(before_shrinking=true,before_merging=false),max_states=50k,threshold_before_merge=1, tokens=-1, preclone=3), astar(h))"])
]
BUILD_OPTIONS = []
